...
function CandyArray(arrayType, index) {
	...
	var candyColumna = $([candyCol1, candyCol2, candyCol3, candyCol4,
...
function candyFila(index) {
	var candyRow = CandyArray('rows', index);
	return candyRow;
}

function candyColumna(index) {
...
function validarColumna() {
	for (var j = 0; j < 7; j++) {
		...
		var candyColumn = candyColumna(j);
		...
		if (candyCount >= 3) {
			eliminarCandyColumna(candyPosition, candyColumn);
			puntuacionPantalla(candyCount);
		}
	}
}

function eliminarCandyColumna(candyPosition, candyColumn) {
	for (var i = 0; i < candyPosition.length; i++) {
		candyColumn.eq(candyPosition[i]).addClass('delete');
	}
}

function validacionEnFila() {
	for (var j = 0; j < 6; j++) {
		var counter = 0;
		var candyPosition = [];
		var extraCandyPosition = [];
		var candyRow = candyFila(j);
		...
		candyCount = candyPosition.length;
		if (candyCount >= 3) {
			borrarCandyHorizontal(candyPosition, candyRow);
			puntuacionPantalla(candyCount);
		}
	}
}

function borrarCandyHorizontal(candyPosition, candyRow) {
	for (var i = 0; i < candyPosition.length; i++) {
		candyRow[candyPosition[i]].addClass('delete');
	}
}

function puntuacionPantalla(candyCount) {
...
function fillTablero() {
	...
	column.each(function () {
		...
		for (var i = 0; i < agrega; i++) {
			var candyType = obtenerAleatorio(1, 5);
			if (i === 0 && candys < 1) {
...
function agregarCandy() {
	$('img').draggable({
		...
		grid: [100, 100],
		zIndex: 10,
		drag: movimientoCandy
	});
	$('img').droppable({
		drop: reemplazarCandyAnterior
	});
	activarCandyEvents();
}

function desactivarCandyEvents() {
	$('img').draggable('disable');
	$('img').droppable('disable');
}

function activarCandyEvents() {
	$('img').draggable('enable');
	$('img').droppable('enable');
}

function movimientoCandy(event, candyDrag) {
...
function reemplazarCandyAnterior(event, candyDrag) {
...
function elementosTableroPromise(result) {
	if (result) {
		elementosTablero();
	}
}
...
function borrarCandyAutomatico() {
	desactivarCandyEvents();
	...
	$('img.delete').animate({
		...
		.animate({
			...
			complete: function () {
				borrarDulces()
					.then(elementosTableroPromise)
					.catch(espaciosVacios);
			},
			queue: true
		});
}

function espaciosVacios(error) {
	console.log(error);
}
